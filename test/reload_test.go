...
func TestReload(t *testing.T) {
	...
	c, err := DNServerServer(corefile)
	if err != nil {
		t.Fatalf("Could not get DNServer serving instance: %s", err)
	}

	udp, _ := DNServerServerPorts(c, 0)
	...
	udp, _ = DNServerServerPorts(c1, 0)
...
func TestReloadSeveralTimeMetrics(t *testing.T) {
	...
	serverWithMetrics, err := DNServerServer(corefileWithMetrics)
	if err != nil {
		if strings.Contains(err.Error(), inUse) {
			return
		}
	...
	for i := 0; i < reloadCount; i++ {
		...
		if err != nil {
			t.Errorf("Could not restart DNServer : %s, at loop %v", err, i)
		}
		if err := collectMetricsInfo(promAddress, proc); err != nil {
			t.Errorf("Prometheus is not listening : %s", err)
		}
	...
	if err != nil {
		t.Errorf("Could not restart a second time DNServer : %s", err)
	}
...
func TestMetricsAvailableAfterReload(t *testing.T) {
	...
	inst, _, tcp, err := DNServerServerAndPorts(corefileWithMetrics)
	if err != nil {
		if strings.Contains(err.Error(), inUse) {
			return
		}
	...
	if err != nil {
		t.Errorf("Could not restart DNServer : %s", err)
		instReload = inst
	}
