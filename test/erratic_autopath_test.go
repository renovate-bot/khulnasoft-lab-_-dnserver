...
func setupProxyTargetDNServer(t *testing.T, fn func(string)) {
	tmpdir := t.TempDir()
...
`
	...
	if err != nil {
		t.Fatalf("Could not get proxy target DNServer serving instance: %s", err)
	}
	defer i.Stop()
...
func TestLookupAutoPathErratic(t *testing.T) {
	setupProxyTargetDNServer(t, func(proxyPath string) {
		corefile := `.:0 {
			erratic
			autopath @erratic
			forward . ` + proxyPath + `
