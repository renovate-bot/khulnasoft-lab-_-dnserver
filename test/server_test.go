...
func TestProxyToChaosServer(t *testing.T) {
	t.Parallel()
	corefile := `.:0 {
		chaos DNServer-001 miek@miek.nl
	}`

	chaos, udpChaos, _, err := DNServerServerAndPorts(corefile)
	if err != nil {
	...
	proxy, udp, _, err := DNServerServerAndPorts(corefileProxy)
	if err != nil {
		t.Fatalf("Could not get DNServer serving instance")
	}
	defer proxy.Stop()
...
func chaosTest(t *testing.T, server string) {
	...
	if r.Answer[0].String() != `version.bind.	0	CH	TXT	"DNServer-001"` {
...
func TestReverseExpansion(t *testing.T) {
	...
	server, udp, _, err := DNServerServerAndPorts(corefile)
	if err != nil {
		t.Fatalf("Could not get DNServer serving instance: %s", err)
	}
...
func TestMultiZoneBlockConfigs(t *testing.T) {
	...
	for j := 0; j < 3; j++ {
		...
		if server, err = DNServerServer(corefile); err != nil {
			continue
		}
		t.Logf("Got running DNServer serving instance, after %d tries", j+1)
		break // success
	}
	if server == nil {
		t.Fatalf("Could not get DNServer serving instance: %s", err)
