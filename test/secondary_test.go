...
func TestSecondaryZoneTransfer(t *testing.T) {
	...
	i, _, tcp, err := DNServerServerAndPorts(corefile)
	if err != nil {
		t.Fatalf("Could not get DNServer serving instance: %s", err)
	}
	defer i.Stop()
	...
	i1, udp, _, err := DNServerServerAndPorts(corefile)
	if err != nil {
		t.Fatalf("Could not get DNServer serving instance: %s", err)
	}
	defer i1.Stop()
...
`)
	...
	master, _, _, err := DNServerServerAndPorts(corefileMaster)
	if err != nil {
		t.Fatalf("Could not start DNServer master: %s", err)
	}
	defer master.Stop()
