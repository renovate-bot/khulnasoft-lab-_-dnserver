+++
title = "DNServer-1.0.0 Release"
description = "DNServer-1.0.0 Release Notes."
...
We are pleased to announce the [release](https://github.com/khulnasoft-lab/dnserver/releases/tag/v1.0.0) of DNServer-1.0.0!

Release 1.0.0 and other recent releases have focused on improving the performance and
functionality of the *kubernetes* plugin, since DNServer is now on track to eventually
[replace kube-dns](https://github.com/kubernetes/features/issues/427) as the default
cluster DNS in Kubernetes.

As part of the [Kubernetes proposal](https://github.com/kubernetes/community/pull/1100), we have shown that DNServer
not only provides more functionality than kube-dns, but performs much better while using less memory. In our tests,
[DNServer](https://github.com/kubernetes/community/pull/1100#issuecomment-337747482) running against a cluster with 5000
...
This can be partial ascribed to DNServer simpler runtime - a single process instead of a combination of several processes.

DNServer also implements a number of Kubernetes-related features that are not part of kube-dns, including:
...
As a general-purpose DNS server, DNServer also enables many other use cases that would be difficult or impossible to
achieve with kube-dns, such as the ability to create [custom DNS entries](https://dnsserver.khulnasoft.com/2017/05/08/custom-dns-entries-for-kubernetes/).

We are excited to continue our contributions to the Kubernetes community, and DNServer is being incorporated as a 1.9 alpha feature into a variety
