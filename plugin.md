...
`ServeDNS` returns two values, a response code and an error. If the error is not nil, DNServer
will return a SERVFAIL to the client. The response code tells DNServer if a *reply has been
written by the plugin chain or not*. In the latter case DNServer will take care of that.
...
See a couple of blog posts on how to write and add plugin to DNServer:

* <https://dnsserver.khulnasoft.com/2017/03/01/how-to-add-plugins-to-coredns/>
* <https://dnsserver.khulnasoft.com/2016/12/19/writing-plugins-for-coredns/>, slightly older, but useful.
...
Plugins for DNServer can live out-of-tree, `plugin.cfg` defaults to DNServer' repo but other
