...
perspective of multiple DNServer instances; each instance still checks on a
...
DNServer starts and serves health from :8080. Now you change `:8080` to `:443` not knowing a process
...
In DNServer v1.6.0 and earlier any `import` statements are not discovered by this plugin.
This means if any of these imported files changes the *reload* plugin is ignorant of that fact.
DNServer v1.7.0 and later does parse the Corefile and supports detecting changes in imported files.
