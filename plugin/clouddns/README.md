...
be used when DNServer is deployed on GCP or elsewhere. Note that this plugin accesses the resource
records through the Google Cloud API. For records in a privately hosted zone, it is not necessary to
place DNServer and this plugin in the associated VPC network. In fact the private hosted zone could
