...
import (
	...
	"github.com/khulnasoft-lab/dnserver/plugin/kubernetes/object"
...
// ExternalAddress returns the external service address(es) for the DNServer service.
func (k *Kubernetes) ExternalAddress(state request.Request, headless bool) []dns.RR {
	// If DNServer is running inside the Kubernetes cluster: k.nsAddrs() will return the external IPs of the services
	// targeting the DNServer Pod.
	// If DNServer is running outside of the Kubernetes cluster: k.nsAddrs() will return the first non-loopback IP
