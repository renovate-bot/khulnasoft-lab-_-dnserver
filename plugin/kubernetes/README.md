...
DNServer running the kubernetes plugin can be used as a replacement for kube-dns in a kubernetes
cluster.  See the [deployment](https://github.com/khulnasoft-lab/deployment) repository for details on [how
to deploy DNServer in Kubernetes](https://github.com/khulnasoft-lab/deployment/tree/master/kubernetes).
...
When DNServer starts with the *kubernetes* plugin enabled, it will delay serving DNS for up to 5 seconds
until it can connect to the Kubernetes API and synchronize all object watches.  If this cannot happen within
5 seconds, then DNServer will start serving DNS while the *kubernetes* plugin continues to try to connect
and synchronize all object watches.  DNServer will answer SERVFAIL to any request made for a Kubernetes record
...
Connect to Kubernetes with DNServer running outside the cluster:
...
packet received by DNServer must be the IP address of the Pod that sent the request.
 ...
 * the remote IP address in the DNS packet received by DNServer must be the IP address
