...
func TestTrace_DOH_TraceHeaderExtraction(t *testing.T) {
	...
	fs := m.FinishedSpans()
	rootDNServerspan := fs[1]
	rootDNServerTraceID := rootDNServerspan.Context().(mocktracer.MockSpanContext).TraceID
	outsideSpanTraceID := outsideSpan.Context().(mocktracer.MockSpanContext).TraceID
	if rootDNServerTraceID != outsideSpanTraceID {
		t.Errorf("Unexpected traceID: rootSpan.TraceID: want %v, got %v", rootDNServerTraceID, outsideSpanTraceID)
	}
}
