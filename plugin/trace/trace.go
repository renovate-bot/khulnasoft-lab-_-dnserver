...
import (
	...
	_ "github.com/khulnasoft-lab/dnserver/plugin/pkg/trace" // Plugin the trace package.
...
var tagByProvider = map[string]traceTags{
	"default": {
		Name:   "dnsserver.khulnasoft.com/name",
		Type:   "dnsserver.khulnasoft.com/type",
		Rcode:  "dnsserver.khulnasoft.com/rcode",
		Proto:  "dnsserver.khulnasoft.com/proto",
		Remote: "dnsserver.khulnasoft.com/remote",
	},
	"datadog": {
		Name:   "dnsserver.khulnasoft.com@name",
		Type:   "dnsserver.khulnasoft.com@type",
...
func (t *trace) ServeDNS(ctx context.Context, w dns.ResponseWriter, r *dns.Msg) (int, error) {
	...
	if !plugin.ClientWrite(status) {
		...
		// see https://github.com/khulnasoft-lab/dnserver/blob/master/core/dnsserver/server.go#L318
		rc = status
	}
